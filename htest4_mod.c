...
int main( int argc, char **argv )
{
  ...
  struct record *bonilla = (struct record *)getElement(performance, by_last_name, &rec);
  removeElement( performance, by_last_name, &rec );

  strcpy( rec.last_name, "Jacobs" );
  struct record *jacobs = (struct record *)getElement(performance, by_last_name, &rec);
  removeElement( performance, by_last_name, &rec );

  struct record toAdd;
  strcpy( toAdd.last_name, "Bonill");
  strcpy( toAdd.first_name, "Hyrum");
  strcpy( toAdd.phone_number, "911-124-7812");
  for (int i = 0; i < 22; i++)
  {
    addElement(performance, by_last_name, &toAdd);
  }

  removeElement(performance, by_last_name, &toAdd);
  // ADDING JACOBS TO OLD BONILLA POSITION
  addElement(performance, by_last_name, jacobs);

  // REMOVING RANDOM VALUE WE ADDED TO OLD JACOBS POSITION
  by_last_name->data[93] = NULL;
  performance->writes++;
  by_last_name->nel--;

  // ADDING BONILLA INTO OLD JACOBS POSTIION
  addElement(performance, by_last_name, bonilla);
