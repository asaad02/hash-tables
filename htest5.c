...
void randomize( struct HashTable *table)
{
    int randInt = 0;

    for (int i = 0; i < table->capacity; i++){
        randInt = (i + (table->capacity / 4) >= table->capacity)? (i + (table->capacity / 4)) - table->capacity : (i + (table->capacity / 4));

        void *tempData = table->data[i];
        table->data[i] = table->data[randInt];
        table->data[randInt] = tempData;
    }
